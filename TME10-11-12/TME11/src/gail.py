...
class GAILAgent(nn.Module):
	...
	def __init__(self, env, opt):
		...
		#---optimizers tools---#
		self.optim_disc = torch.optim.Adam(params=self.discriminator.parameters(), lr=lr)
	...
	def train(self, beta):
		...
		adv_loss = torch.log(self.discriminator(x_expert)) + torch.log(one - self.discriminator(x_agent))
		...
		r_p = torch.where(r_p > -100., r_p, -100.)
		...
		critic_loss = F.smooth_l1_loss(v_s, adv.detach())             # what permits a better judgment
		...
		for _ in range(K):
			pi = self.pi(s)
			prob = pi.gather(1,a.view(1,-1)).flatten()                  # calculate the new probabilities for the actions already taken
			# print("prob.shape:", prob.shape)
			# print("prob_sample.shape:", prob_sample.shape)
			ratio = torch.exp(torch.log(prob) - torch.log(prob_sample))     # a/b = exp(log(a)-log(b))
			# print("ratio:", ratio)
		...
		with torch.no_grad():
			...
			d_kl = (prob_new_actions * (torch.log(prob_new_actions)/\
				torch.log(prob_sample))).mean()                             # KL div btw theta k et theta k+1
