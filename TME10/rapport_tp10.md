...
Dans cet environnement, les agents ne semblent apprendre que très peu, ou du moins de manière très lente. Comme on peut le voir dans la figure ci-dessous, l'agent commence d'apport à perdre en nombre quantité de récompenses. 
...
Néanmoins, on note bien une descente de gradient du coût TD. L'erreur augmente lors du remplissage du *replay buffer* puis commence à descendre. La recherche d'hyperparamètres par optimisation automatique (framework *Optuna*) n'a pas donnée de meilleurs résultats.
...
On peut voir que l'agent apprend correctement sur cet environnement bien que les agents manquents de stabilité dans leur récompenses à long terme. Il est important de visualiser la récompenses total des agents car elles sont spécifiques par individu dans cet environnement. Par ailleurs, il est intéressant de noter que les récompenses sont plutôt similaires entre l'agent 0 et les deux autres agents 1 et 2 qui sont adversaires. Les récompenses semblent ainsi relativement bien équilibrés pour éviter qu'un des agents prenne le dessus.
...
L'apprentissage du modèle sur cette environnement n'est pas certaine comme l'atteste la figure ci-dessous :
